...
import { DiceConsts } from "./die-consts";
import { calculateTextureSize, copyto, createDiceGeometry, GeometryWithCannonShape } from "./dicelib";
import { Mesh, MeshPhongMaterial, Object3D, Quaternion, Texture, Vector3 } from "three";
...
export abstract class Die extends Object3D {
  ...
  get body(): Body {
    if (!this._body) {
      ...
      if (this.geometry && this.geometry.cannonShape) {
  ...
  geometry: GeometryWithCannonShape | undefined;
  ...
  get value() {
    ...
    //   for (let i = 0, l = this.geometry.; i < l; ++i) {
    //     const face = this.geometry.[i];
