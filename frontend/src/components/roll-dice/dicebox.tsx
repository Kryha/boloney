...
//     let ambientLight = new AmbientLight(this.ambientLightColor);
...
//     let body = new Body({ mass: 0, material: this.deskBodyMaterial });
...
//     let box = this;
//     bind(container, ["mousedown", "touchstart"], (ev: TouchEvent | MouseEvent) => {
//       ev.preventDefault();
//       box.mouseTime = new Date().getTime();
//       box.mouseStart = this.getMousePosition(ev);
//     });
//     bind(container, ["mouseup", "touchend"], (ev: TouchEvent | MouseEvent) => {
...
//         let boost = Math.sqrt((2500 - interval) / 2500) * dist * 2;
//         initRng(() => {
...
//   bindThrow(button, notationGetter: NotationGetter, beforeRoll: BeforeDiceRollCallback, afterRoll: AfterDiceRollCallback) {
//     const box = this;
//     bind(button, ["mouseup", "touchend"], (ev: MouseEvent | TouchEvent) => {
//       ev.stopPropagation();
...
//   findDieAtMousePosition(ev: MouseEvent | TouchEvent) {
//     const m = this.getMousePosition(ev);
//     let intersects = new Raycaster(
...
//   roll(vectors: DieVector[], values, callback) {
...
//     const roll = (requestResults: boolean = false) => {
//       if (afterRoll) {
//         this.clear();
//         this.roll(vectors, requestResults || notation.result, (result) => {
//           if (afterRoll) {
...
//     let vectors: DieVector[] = [];
//     for (let sides of notation.set) {
...
//   private addDice(sides: number, pos, velocity, angle, axis) {
//     const die = this.diceFactory.createDie(sides);
...
//         let a = die.body.angularVelocity,
//           v = die.body.velocity;
...
//       let interact = <Die>this.scene.children[i];
...
//     let dAngle = (0.3 * timeDiff * Math.PI * Math.min(24000 + threadid - time, 6000)) / 6000;
