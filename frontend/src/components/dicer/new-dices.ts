...
import { DiceD6, DiceManager, DiceObjectClass } from "./threejs-dice";
...
const dice: typeof DiceObjectClass[] = [];

init();

// interface DiceObjectValue {
//   dice: DiceObjectClass;
//   value: number;
...
function init() {
  ...
  for (let i = 0; i < 5; i++) {
    const die = new DiceD6({ size: 3.5, backColor: colors[i] });
  ...
  function randomDiceThrow() {
    ...
    for (let i = 0; i < dice.length; i++) {
      ...
      if (dice[i] != null) {
        dice[i].getObject()!.velocity.set(25 + rand, 40 + yRand, 15 + rand);
