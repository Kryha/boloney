...
declare namespace nkruntime {
  ...
  export interface MatchJoinFunction<State = MatchState> {
     ...
     */
    (ctx: Context, logger: Logger, nk: Nakama, dispatcher: MatchDispatcher, tick: number, state: State, presences: Presence[]): {
      state: State;
    } | null;
  }
  ...
  export interface MatchLoopFunction<State = MatchState> {
     ...
     */
    (ctx: Context, logger: Logger, nk: Nakama, dispatcher: MatchDispatcher, tick: number, state: State, messages: MatchMessage[]): {
      state: State;
    } | null;
  }
  ...
  export interface MatchTerminateFunction<State = MatchState> {
     ...
     */
    (ctx: Context, logger: Logger, nk: Nakama, dispatcher: MatchDispatcher, tick: number, state: State, graceSeconds: number): {
      state: State;
    } | null;
  }
  ...
  export interface MatchSignalFunction<State = MatchState> {
     ...
     */
    (ctx: Context, logger: Logger, nk: Nakama, dispatcher: MatchDispatcher, tick: number, state: State, data: string): {
      state: State;
      data?: string;
    } | null;
  }
  ...
  export interface Initializer {
    ...
    registerAfterListLeaderboardRecordsAroundOwner(
      fn: AfterHookFunction<LeaderboardRecordList, ListLeaderboardRecordsAroundOwnerRequest>
    ): void;
    ...
    registerAfterListTournamentRecordsAroundOwner(
      fn: AfterHookFunction<LeaderboardRecordList, ListTournamentRecordsAroundOwnerRequest>
    ): void;
  ...
  type Envelope =
    | EnvelopeChannel
    | EnvelopeChannelJoin
    | EnvelopeChannelLeave
    | EnvelopeChannelMessageSend
    | EnvelopeChannelMessageUpdate
    | EnvelopeChannelMessageRemove
    | EnvelopeMatchCreateMessage
    | EnvelopeMatchDataSend
    | EnvelopeMatchJoin
    | EnvelopeMatchLeave
    | EnvelopeMatchmakerAdd
    | EnvelopeMatchmakerRemove
    | EnvelopeStatusFollow
    | EnvelopeStatusUnfollow
  ...
  export interface Nakama {
     ...
     */
    multiUpdate(
      accountUpdates: UserUpdateAccount[] | null,
      storageObjectsUpdates: StorageWriteRequest[] | null,
      walletUpdates: WalletUpdate[] | null,
      updateLedger?: boolean
    ): { storageWriteAcks: StorageWriteAck[]; walletUpdateAcks: WalletUpdateResult[] };
